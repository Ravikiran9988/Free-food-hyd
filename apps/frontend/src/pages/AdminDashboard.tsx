...
type Tab = 'overview' | 'submissions' | 'reports' | 'updates' | 'activity' | 'users';
...
export function AdminDashboard() {
  ...
  const loadAllData = useCallback(async () => {
    ...
    setError(null);
    try {
      const [statsData, submissionsData, reportsData, updatesData, activityData, usersData] =
        await Promise.all([
          fetchStats(),
  ...
  const filteredUsers = users.filter((u) =>
    userSearch.trim() ? u.email.toLowerCase().includes(userSearch.toLowerCase()) : true
  );
          ...
          <section className="space-y-6">
            ...
            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 mt-4">
                ...
                <ActionSummary title="Registered Users" count={users.length} onClick={() => setActiveTab('users')} />
              </div>
            </div>
          </section>
        )}
          ...
          <section className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
            <div className="p-5 sm:p-6 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div>
                ...
                <p className="text-sm text-slate-500 mt-1">
                  View registered users, manage administrative roles, and remove accounts.
                </p>
              </div>
              ...
              <div className="relative w-full sm:w-72">
                <input
                  ...
                  className="w-full pl-9 pr-4 py-2 rounded-xl border border-slate-200 text-sm text-slate-900 focus:outline-none focus:ring-2 focus:ring-brand-500"
                />
                ...
                <table className="w-full text-left text-sm text-slate-600">
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {filteredUsers.map((u) => {
                      ...
                      const isTargetLoading = userActionLoading === u.id;
                          ...
                          <td className="px-6 py-4">
                            <div className="flex items-center gap-3">
                              ...
                              <div>
                                ...
                                <p className="text-xs text-slate-400 mt-0.5 font-mono">ID: {u.id}</p>
                              </div>
                            </div>
                          </td>
                            ...
                            {isConfirmingDelete ? (
                              <div className="flex items-center justify-end gap-2">
                                ...
                                <button
                                  ...
                                  className="px-2.5 py-1 rounded-lg bg-red-600 hover:bg-red-700 text-white text-xs font-bold transition-colors disabled:opacity-50"
                                >
                                  {isTargetLoading ? <Loader2 className="w-3 h-3 animate-spin" /> : 'Confirm'}
                                </button>
                                <button
                                  disabled={isTargetLoading}
                              ...
                              <div className="flex items-center justify-end gap-2">
                                <button
                                  disabled={isTargetLoading || (isSelf && u.role === 'admin')}
                                ...
                                <button
                                  disabled={isTargetLoading || isSelf}
                                  onClick={() => setDeleteConfirmUserId(u.id)}
                                  title={isSelf ? 'Cannot delete your own account' : 'Delete user'}
                                  className="inline-flex items-center p-1.5 rounded-lg text-slate-400 hover:text-red-600 hover:bg-red-50 transition-colors disabled:opacity-30 disabled:cursor-not-allowed"
                                >
