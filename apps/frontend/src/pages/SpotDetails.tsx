...
export function SpotDetails() {
  ...
  return (
    <div className="bg-slate-50 min-h-screen pb-24 md:pb-16">
      {/* Header/Hero area */}
      <div className="bg-white border-b border-slate-200/90 pt-5 pb-7">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          <button 
            onClick={() => navigate(-1)} 
            className="inline-flex items-center gap-2 text-slate-500 hover:text-slate-900 mb-5 transition-colors font-medium text-sm py-1 px-2 -ml-2 rounded-lg hover:bg-slate-100"
          >
          ...
          <div className="flex flex-col md:flex-row md:items-start justify-between gap-6">
            <div className="flex-1">
              <div className="flex flex-wrap items-center gap-2 mb-3">
                <span className="text-[11px] font-semibold tracking-wider text-brand-700 uppercase bg-brand-50 border border-brand-100 px-2.5 py-0.5 rounded-full">
                  {spot.category || 'Annadhanam'}
                </span>
                <StatusBadge status={liveStatus} />
                <span className="text-xs font-medium text-slate-500 bg-slate-100 px-2.5 py-0.5 rounded-full">
                  {spot.source_type === 'imported' ? 'Source Verified' : 'Community Submission'}
                </span>
              </div>
              <h1 className="text-2xl sm:text-3xl md:text-4xl font-extrabold text-slate-900 mb-3 leading-tight tracking-tight">
              ...
              <p className="text-sm sm:text-base text-slate-600 flex items-start gap-2 leading-relaxed">
                <MapPin className="w-4 h-4 mt-1 shrink-0 text-brand-600" />
                <span>{spot.area_name}</span>
              </p>
            </div>
            ...
            <div className="hidden md:flex flex-wrap items-center gap-3 shrink-0">
              <button 
                ...
                className="flex items-center gap-1.5 bg-white hover:bg-slate-50 text-slate-700 border border-slate-200 px-4 py-2.5 rounded-xl font-semibold text-xs sm:text-sm shadow-xs transition-colors"
              >
              ...
              {mapsUrl && (
                <a 
                  ...
                  className="flex items-center gap-2 bg-brand-600 hover:bg-brand-700 text-white px-5 py-2.5 rounded-xl font-semibold text-xs sm:text-sm shadow-xs hover:shadow transition-all"
                >
            ...
            <div className="flex md:hidden items-center gap-2.5 pt-1">
              {mapsUrl && (
                <a 
                  ...
                  target="_blank" 
                  rel="noopener noreferrer"
                  className="flex-1 flex items-center justify-center gap-2 bg-brand-600 hover:bg-brand-700 text-white px-4 py-2.5 rounded-xl font-bold text-sm shadow-xs transition-all"
                >
                  <Navigation className="w-4 h-4" />
              ...
              <button 
                onClick={() => setShowUpdateModal(true)}
                className="px-3 py-2.5 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-700 font-semibold text-xs transition-colors flex items-center gap-1.5"
              >
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 mt-6 sm:mt-8 grid grid-cols-1 md:grid-cols-3 gap-6 sm:gap-8">
        
        {/* Main Content Column */}
        <div className="md:col-span-2 space-y-6">
          
          {/* REAL-TIME COMMUNITY CONFIRMATION ACTION BOX */}
          <section className="bg-white rounded-2xl p-5 sm:p-6 shadow-2xs border border-slate-200/90">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4 pb-4 border-b border-slate-100">
              <div>
                <div className="flex items-center gap-2 mb-1">
                  <StatusBadge status={liveStatus} size="sm" />
                  <span className="text-xs text-slate-500 font-medium">Live Community Status</span>
                </div>
                <h3 className="text-base sm:text-lg font-bold text-slate-900">
                  {liveStatus === 'expired' ? 'This event has concluded' :
                   liveStatus === 'serving_now' ? 'Is this food being served right now?' :
                   liveStatus === 'starting_soon' ? 'Is this distribution starting soon?' :
                   'Is this food available today?'}
                </h3>
              </div>
              <div className="sm:text-right">
                <span className="inline-block text-xs font-semibold text-emerald-800 bg-emerald-50 px-3 py-1 rounded-lg border border-emerald-200/80">
                  {confirmation?.confirmation_text || 'No recent attendee updates'}
                </span>
              </div>
            </div>
            ...
            {liveStatus === 'serving_now' ? (
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                <button
                  ...
                  className="bg-emerald-600 hover:bg-emerald-700 text-white p-3 rounded-xl font-bold text-xs flex flex-col items-center gap-1 transition-all disabled:opacity-50 shadow-xs"
                ...
                <button
                  ...
                  className="bg-slate-100 hover:bg-red-50 hover:text-red-700 text-slate-700 p-3 rounded-xl font-bold text-xs flex flex-col items-center gap-1 transition-all disabled:opacity-50 border border-slate-200"
                ...
                <button
                  ...
                  className="bg-slate-100 hover:bg-amber-50 hover:text-amber-800 text-slate-700 p-3 rounded-xl font-bold text-xs flex flex-col items-center gap-1 transition-all disabled:opacity-50 border border-slate-200"
                >
                ...
                <button
                  ...
                  className="bg-slate-100 hover:bg-slate-200 text-slate-700 p-3 rounded-xl font-bold text-xs flex flex-col items-center gap-1 transition-all disabled:opacity-50 border border-slate-200"
                >
              ...
              <div className="grid grid-cols-2 gap-3">
                <button
                  ...
                  className="bg-emerald-600 hover:bg-emerald-700 text-white p-3.5 rounded-xl font-bold text-xs sm:text-sm flex items-center justify-center gap-2 transition-all disabled:opacity-50 shadow-xs"
                  aria-label="Report that the scheduled food event is currently happening"
                >
                  <ThumbsUp className="w-4 h-4" />
                  <span>👍 Yes, happening</span>
                </button>
                <button
                  disabled={feedbackLoading}
                  onClick={() => handleQuickFeedback('cancelled')}
                  className="bg-slate-100 hover:bg-red-50 hover:text-red-700 text-slate-700 p-3.5 rounded-xl font-bold text-xs sm:text-sm flex items-center justify-center gap-2 transition-all disabled:opacity-50 border border-slate-200"
                  aria-label="Report that the food is not happening"
                >
                  <XCircle className="w-4 h-4 text-red-500" />
                  <span>❌ Cancelled</span>
              ...
              <div className="flex flex-col gap-1 text-slate-500 text-xs sm:text-sm">
                <p>This scheduled distribution has concluded for today.</p>
              </div>
            ) : (
              <div className="grid grid-cols-2 gap-3">
                <button
                ...
                <button
                  disabled={feedbackLoading}
                  onClick={() => setShowUpdateModal(true)}
                  className="bg-slate-100 hover:bg-slate-200 text-slate-700 border border-slate-200 p-3.5 rounded-xl font-bold text-xs sm:text-sm flex items-center justify-center gap-2 transition-all"
                >
                  <Edit3 className="w-4 h-4 text-slate-500" />
                  <span>Suggest Timing</span>
                </button>
              </div>
            )}
          </section>
